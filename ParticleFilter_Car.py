...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        # This should also work if noise is added...not sure why Thrun asked not to add noise
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        #2 types of motions for the "bicycle" model
        #1: Moving straight, turning angle beta < 0.001
        #2: Turning, beta > 0.001
        #1:
        #motion = [bearing, distance]
        d = random.gauss(motion[1], self.distance_noise);
        alpha = random.gauss(motion[0], self.steering_noise) % 2*pi;
        beta = (d*tan(alpha)/self.length)
        newTheta = (self.orientation + beta)%(2*pi)
        if abs(beta) < 0.001:
            #going straight
            newX = self.x + d*cos(self.orientation)
            newY = self.y + d*sin(self.orientation)
            
        else:
            #Turning
            #Find center of turning, then find new distance
            R = d/beta;
            Cx = self.x - (sin(self.orientation)*R)
            Cy = self.y + (cos(self.orientation)*R)
            newX = Cx + sin(newTheta)*R
            newY = Cy - cos(newTheta)*R
            
        res = robot();
        res.length = self.length;
        res.set_noise(bearing_noise, steering_noise, distance_noise)
        res.set(newX, newY, newTheta)
        #print "Res: ", res
        #print "Self: ",self
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for L in landmarks:
            delY = L[0] - self.y;
            delX = L[1] - self.x;
            bearing = (atan2(delY, delX) - self.orientation) % (2*pi)
            if add_noise :
...
motions = [[0, 0] for row in range(number_of_iterations)]

#generate measurements and robots final pose.
